...
class NamedShareMemQueue:
    ...
    def __init__(self, gpu_id: int, slot_size: int = 20 * 1024 * 1024,
                 num_slots: int = 128, create: bool = False):
        """
    ...
    def _initialize_resources(self, create: bool):
        ...
        try:
            self.empty = posix_ipc.Semaphore(
                self._empty_sem_name,
                flags=flags,
                initial_value=self.num_slots
            )
            if create:
    ...
    def get(self, slot_idx: Optional[int] = None,
