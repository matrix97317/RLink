...
class RLinkLearner:
    ...
    async def _start_ucxx_server(self):
        ...
        try:
            ...
            # 保持服务器运行
            while self._running and self._learner_data_server and not self._learner_data_server.closed:
        ...
        async def data_probe(data: bytes = Body(...)):
            ...
            try:
                ...
                self._actor_info[actor_id] = {
                    ...
                    "probe_count": self._actor_info.get(actor_id, {}).get("probe_count", 0) + 1
                }
